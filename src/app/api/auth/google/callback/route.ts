import { NextRequest, NextResponse } from "next/server";
import { OAuth2Client } from "google-auth-library";
import { Resend } from "resend";
import pool from "@/lib/db";
import { signToken, signPendingToken } from "@/lib/auth";
import { generateOtpCode, createOtp, getClientIp, logAdminAction } from "@/lib/admin-auth";

const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || "";
const GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET || "";
const ADMIN_EMAIL = (process.env.ADMIN_EMAIL || "").toLowerCase();
const RESEND_API_KEY = process.env.RESEND_API_KEY || "";
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL || "https://diplom-chi-ten.vercel.app";

const resend = RESEND_API_KEY ? new Resend(RESEND_API_KEY) : null;

function redirect(path: string) {
  return NextResponse.redirect(`${SITE_URL}${path}`);
}

export async function GET(req: NextRequest) {
  const ip = getClientIp(req);
  const { searchParams } = req.nextUrl;
  const code = searchParams.get("code");
  const state = searchParams.get("state");
  const storedState = req.cookies.get("oauth_state")?.value;

  const clearState = (res: NextResponse) => {
    res.cookies.set("oauth_state", "", { maxAge: 0, path: "/" });
    return res;
  };

  if (!state || !storedState || state !== storedState) {
    return redirect("/auth?error=invalid_state");
  }

  let redirectTo = "/dashboard";
  try {
    const decoded = JSON.parse(Buffer.from(state, "base64url").toString());
    if (typeof decoded.redirect === "string" && decoded.redirect.startsWith("/")) {
      redirectTo = decoded.redirect;
    }
  } catch { /* keep default */ }

  if (!code) return clearState(redirect("/auth?error=no_code"));

  // Exchange code for ID token
  let email: string | undefined;
  let name: string | undefined;
  try {
    const client = new OAuth2Client(
      GOOGLE_CLIENT_ID,
      GOOGLE_CLIENT_SECRET,
      `${SITE_URL}/api/auth/google/callback`
    );
    const { tokens } = await client.getToken(code);
    const ticket = await client.verifyIdToken({ idToken: tokens.id_token!, audience: GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload();
    email = payload?.email?.toLowerCase();
    name = payload?.name;
    if (!payload?.email_verified || !email) throw new Error("Email not verified");
  } catch (err) {
    console.error("Google callback error:", err);
    return clearState(redirect("/auth?error=google_failed"));
  }

  try {
    // Admin: bootstrap role + send OTP
    if (email === ADMIN_EMAIL) {
      let result = await pool.query("SELECT id, name, email, role FROM users WHERE email = $1", [email]);
      if (result.rows.length === 0) {
        result = await pool.query(
          "INSERT INTO users (name, email, password_hash, role) VALUES ($1, $2, NULL, 'admin') RETURNING id, name, email, role",
          [name?.trim() || email.split("@")[0], email]
        );
      } else if (result.rows[0].role !== "admin") {
        await pool.query("UPDATE users SET role = 'admin' WHERE id = $1", [result.rows[0].id]);
        result.rows[0].role = "admin";
      }

      const user = result.rows[0];

      if (!resend) return clearState(redirect("/auth?error=email_not_configured"));

      const otpCode = generateOtpCode();
      const otpId = await createOtp(user.id, otpCode);

      await resend.emails.send({
        from: "interview.ai <onboarding@resend.dev>",
        to: email,
        subject: `Код входа: ${otpCode}`,
        text: `Код подтверждения: ${otpCode}\n\nДействует 5 минут.`,
      }).catch((err) => console.error("Failed to send OTP:", err));

      await logAdminAction("admin_google_otp_sent", ip, { email });

      const pendingToken = signPendingToken(user.id, otpId);
      const res = redirect(`/auth?step=otp&redirect=${encodeURIComponent(redirectTo)}`);
      res.cookies.set("auth_pending", pendingToken, {
        httpOnly: true,
        secure: process.env.NODE_ENV === "production",
        sameSite: "lax",
        path: "/",
        maxAge: 5 * 60,
      });
      return clearState(res);
    }

    // Regular user: find or create, issue session
    let result = await pool.query("SELECT id, name, email FROM users WHERE email = $1", [email]);
    if (result.rows.length === 0) {
      result = await pool.query(
        "INSERT INTO users (name, email, password_hash, role) VALUES ($1, $2, NULL, 'user') RETURNING id, name, email",
        [name?.trim() || email.split("@")[0], email]
      );
    }

    const user = result.rows[0];
    const token = signToken({ userId: user.id, email: user.email });

    const res = redirect(redirectTo);
    res.cookies.set("token", token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
      sameSite: "lax",
      maxAge: 60 * 60 * 24 * 7,
      path: "/",
    });
    return clearState(res);
  } catch (err) {
    console.error("Google auth DB error:", err);
    return clearState(redirect("/auth?error=server_error"));
  }
}
